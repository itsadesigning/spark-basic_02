...
function addToSum(value) {
  ...
  if (value === "Reset") { // Vrijednost mora biti tocno "Reset" 
    sumElement.innerText = 0;
  } else if (parsedValue >= 1 && parsedValue <= 25) { // Vrijednost mora biti veca ili jednaka od 1 i manja ili jednaka od 25 (1 - 25);
    console.log("Good job for betting rationally.");
    sumElement.innerText = currentSum + parsedValue;
  } else if (parsedValue > 25 && parsedValue <= 100) { // Vrijednost mora biti veca od 25 i manja ili jednaka od 100 (1 - 100);
    console.log("Not sure if that's a good idea, but go for it.");
    sumElement.innerText = currentSum + parsedValue;
  } else if (parsedValue > 100 && parsedValue <= 250) { // Vrijednost mora biti veca od 100 i manja ili jednaka od 250 (1 - 250);
    console.log("Hold it right there champ!");
  } else if (parsedValue === 500) { // Vrijednost mora tocno 500 (i tip i vrijednost, i.e. '500' ne bi radilo);
    console.log("It's time to leave sir.")
  } else { // U slucaju da niti jedan od gornjih uvjeta nije zadovoljen izvrsi ovo.
    console.log("Something went wrong with the amount you wanted to insert. VALUE: ", parsedValue);
  }
}
